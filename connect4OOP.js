...
let gb = Array.from(gameboard);
		...
		///  Title: Building a connect-four game with JavaScript                                       ///
...
drop.addEventListener('click', handleClick);
	...
	function handleClick(event) {
		...
		function placeInTable(y, x) {
			...
			console.log(p2Color);
			console.log(disc);
			if (disc.classList.contains('p1')) {
				console.log('player 1');
				disc.style.backgroundColor = `${p1Color}`;
			}else if (disc.classList.contains('p2')) {
				console.log('player 2');
				disc.style.backgroundColor = `${p2Color}`;
			}
		}

		currPlayer = this.currPlayer = this.currPlayer === 1 ? 2 : 1;
	...
	function checkWhoWon() {
		...
		for (let i = 0; i < winArray.length; i++) {
			...
			if (winArray[i].some(isEmpty)) {
				//.some boolean; .find returns a value
				console.log('Empty');
				continue;
			}
